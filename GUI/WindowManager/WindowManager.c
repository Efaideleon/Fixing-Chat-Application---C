...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		while (gtk_events_pending())
		{
			...
			if (chat_window_ui->credential_service->signInvalid == TRUE && chat_window_ui->OpenDialog == 0 && friend_list_ui != NULL) // gotta set OpenDialog to 1 to receive messages
			{
				if ((*seconds_rn % 2) == 0)
				{
				...
				{
					chat_window_ui->OpenDialog = 1;
					open_dialog(chat_window_ui->OpenDialog, application_manager->network_service->SocketFD);
					strncpy(response, "", sizeof(response));
				}
			}

			if ((chat_window_ui != NULL) && (chat_window_ui->OpenDialog == 0))
			{
				if ((*seconds_rn % 2) == 0)
				{
					...
					{
						show_open_dialog(application_manager->message_service->match, application_manager->friend_request_handler->contacts, application_manager->message_service->response, &application_manager->message_service->OpenDialog, application_manager->network_service->SocketFD, application_manager->credential_service->username, application_manager->message_service->messager_buffer, application_manager->message_service->current_page_i);
					}
				}
			}
			gtk_main_iteration();
